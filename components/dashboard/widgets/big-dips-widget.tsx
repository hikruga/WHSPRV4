...
export function BigDipsWidget({ onCoinClick, gridLayout }: BigDipsWidgetProps) {
  ...
  const updateLayout = useCallback(() => {
    ...
    // Calculate columns based on container width (pixel-based)
    const minColumnWidth = 280
    const calculatedColumns = Math.max(1, Math.floor(containerWidth / minColumnWidth))
    
    // Calculate rows based on grid layout height (grid-based)
    const itemHeight = 52 // Even more aggressive to fit more tokens
    const gapSize = 4 // Further reduced to save space
    const containerPadding = 4 // Minimal padding to maximize space
    
    let availableGridHeight = 240 // Default fallback
    if (gridLayout?.h) {
      // Use grid layout height: gridRows * rowHeight 
      availableGridHeight = gridLayout.h * 60 // 60 is the rowHeight from react-grid-layout
    }
    
    const availableHeight = availableGridHeight - containerPadding
    ...
    const totalVisible = Math.min(maxTokens, tokensData.length)
  ...
  return (
    ...
    >
      {tokensData.slice(0, visibleTokens).map((token) => (
        <div
          key={token.symbol}
          className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-orange-500/10 to-transparent border border-orange-500/20 cursor-pointer hover:bg-orange-500/20 transition-all duration-200"
        ...
        >
          ...
          {selectedTokenForTrading === token.symbol ? (
            <div className="flex flex-col items-end gap-2 flex-shrink-0">
              ...
              <div className="flex items-center gap-2 bg-black/40 rounded-lg p-2 border border-orange-500/20">
                {/* Axiom */}
                <Tooltip>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <div className="text-right">
                <div className="text-xs text-orange-300">Volume</div>
                <div className="text-xs text-orange-400 font-semibold">{token.volume}</div>
              </div>
              <div className="text-right">
                <div className="text-xs text-orange-300">MC</div>
                <div className="text-xs text-orange-400 font-semibold">{token.mcap}</div>
              </div>
              <div className="text-right">
                <div className="text-xs text-orange-300">{token.change}</div>
                <div className="text-xs text-orange-400 font-semibold">{token.price}</div>
              </div>
              <Button
                size="sm"
                variant="outline"
